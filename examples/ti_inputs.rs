...
impl Threats {
    pub fn new() -> Self {
        const DIAGS: [u64; 15] = [
            0x0100_0000_0000_0000, 0x0201_0000_0000_0000, 0x0402_0100_0000_0000, 0x0804_0201_0000_0000,
            0x1008_0402_0100_0000, 0x2010_0804_0201_0000, 0x4020_1008_0402_0100, 0x8040_2010_0804_0201,
            0x0080_4020_1008_0402, 0x0000_8040_2010_0804, 0x0000_0080_4020_1008, 0x0000_0000_8040_2010,
            0x0000_0000_0080_4020, 0x0000_0000_0000_8040, 0x0000_0000_0000_0080,
        ];
        ...
        let bishop = PieceThreatsData::new(Piece::BISHOP, [Piece::PAWN, Piece::KNIGHT, Piece::BISHOP, Piece::ROOK], |sq| {
            let rank = sq / 8;
            let file = sq % 8;
        ...
        let mut offsets = [6 * 84; 5]; // Patched to 6 * 84 because pawn targets PAWN, KNIGHT, ROOK (3 pieces * 2 colors = 6 targets)
        for (i, &cnt) in [10 * knight.count, 8 * bishop.count, 8 * rook.count, 10 * queen.count].iter().enumerate() {
            offsets[i + 1] = offsets[i] + cnt;
        }

        Self {
            pawn_map: make_targets([Piece::PAWN, Piece::KNIGHT, Piece::ROOK]), // PATCHED
