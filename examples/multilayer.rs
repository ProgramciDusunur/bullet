...
#[derive(Clone, Copy)]
struct HmcInputs(ChessBucketsMirrored);

impl SparseInputType for HmcInputs {
    type RequiredDataType = ChessBoard;

    fn num_inputs(&self) -> usize {
        self.0.num_inputs() + HMC_BUCKETS
    }
    ...
    fn map_features<F: FnMut(usize, usize)>(&self, pos: &Self::RequiredDataType, mut f: F) {
        self.0.map_features(pos, &mut f);
        if pos.extra[0] >= 14 {
            let hmc = (((pos.extra[0] - 14) / 8) as usize).min(10);
            let feat = self.0.num_inputs() + hmc;
            f(feat, feat);
        }
    }

    fn shorthand(&self) -> String {
        format!("{}hmc", self.0.shorthand())
    }

    fn description(&self) -> String {
        format!("{} with HMC", self.0.description())
    }
}
...
fn main() {
    let mut trainer = ValueTrainerBuilder::default()
        .dual_perspective()
        .optimiser(AdamW)
        .inputs(HmcInputs(ChessBucketsMirrored::new(BUCKET_LAYOUT)))
        .output_buckets(MaterialCount::<NUM_OUTPUT_BUCKETS>)
        .save_format(&[
            SavedFormat::id("l0w")
                .transform(|store, weights| {
                    let piece_count = NUM_INPUT_BUCKETS * 768 * HIDDEN_SIZE;
                    let factoriser = store.get("l0f").values.f32().repeat(NUM_INPUT_BUCKETS);
                    weights[..piece_count].iter().zip(factoriser).map(|(a, b)| a + b).collect()
                })
                .round()
                .quantise::<i16>(Q0),
        ...
        .build(|builder, stm_inputs, ntm_inputs, buckets| {
            ...
            let zeros = builder.new_constant(
                (HIDDEN_SIZE * HMC_BUCKETS, 1),
                &vec![0.0; HIDDEN_SIZE * HMC_BUCKETS],
            );
            let full_factoriser = expanded_factoriser
                .reshape((HIDDEN_SIZE * 768 * NUM_INPUT_BUCKETS, 1))
                .concat(zeros)
                .reshape((HIDDEN_SIZE, 768 * NUM_INPUT_BUCKETS + HMC_BUCKETS));

            let mut l0 = builder.new_affine("l0", 768 * NUM_INPUT_BUCKETS + HMC_BUCKETS, HIDDEN_SIZE);
            l0.weights = l0.weights + full_factoriser;
